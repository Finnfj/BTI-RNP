...
int sendFile(char* filename) {
    ...
    char* ptr;
    ptr = strtok(buffer, " ");
    // print response, server hostname and server ip
    printf("%s\nServer: %s\nServer-IP: %s\n", ptr, srv_hostname, srv_ip);
    // print date of response
    ptr = strtok(NULL, " ");
    printf("%s", ptr);
    ptr = strtok(NULL, " ");
    printf(" %s\n", ptr);
...
int connectToServer(char* address) {
    ...
    {
        struct in_addr* serverAddress = &(((struct sockaddr_in*) (p->ai_addr))->sin_addr);
        srv_ip = (char *) malloc(INET_ADDRSTRLEN);
        char addrstrbuf[INET_ADDRSTRLEN] = "";
        inet_ntop(AF_INET, serverAddress, addrstrbuf, sizeof(addrstrbuf));
        strcpy(srv_ip, addrstrbuf);
        hostaddrlen = sizeof(struct sockaddr_in);
    }
    else // AF_INET6
    {
        struct in6_addr* serverAddress = &(((struct sockaddr_in6*) (p->ai_addr))->sin6_addr);
        srv_ip = (char *) malloc(INET6_ADDRSTRLEN);
        char addrstrbuf[INET6_ADDRSTRLEN] = "";
        inet_ntop(AF_INET6, serverAddress, addrstrbuf, sizeof(addrstrbuf));
        strcpy(srv_ip, addrstrbuf);
        hostaddrlen = sizeof(struct sockaddr_in6);
    }
    // get and save hostname
    status = 0;
    status = getnameinfo(p->ai_addr, hostaddrlen, srv_hostname, sizeof(srv_hostname), NULL, 0, 0);
    if (0 != status) {
        fprintf(stderr, "Error: %s\n", gai_strerror(status)); // gai_strerror is getaddrinfo's Error descriptor
        fflush(stderr);
        return -1;
